...
Registrar.prototype.createBid  = function(name, account, max, secret, callback) {
  if (this.available(name)) {
    if (!this.upForAuction(name)) {
      ...
      var bid = this.registrar.shaBid(this.web3.sha3(name), account, this.web3.toWei(max, 'ether'), secret);
      this.registrar.newBid(bid, {from: account, value: this.web3.toWei(max, 'ether'), gas: 1000000}, callback);
    }
  } else {
